...
public interface StudentCourseDao {
    ...
    final String GET_STUDENT_COURSES = """
                                               SELECT m.mark_id, s.student_id, s.name AS student_name, c.course_id, c.crse_name, m.mark
                                               FROM student s
                                               INNER JOIN marks m ON m.student_id = s.student_id
                                               INNER JOIN course c ON c.course_id = m.course_id""";
    final String GET_STU_CRSE_BYID =  "SELECT * FROM `marks` WHERE `mark_id` = ?";
    final String DELETE_RECORD = "DELETE FROM `marks` WHERE `mark_id` = ?";
